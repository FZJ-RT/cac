...
#Train tfunet
def train_tfunet(training_images, modification_value, training_labels, 
                 model, batch_size, epochs,
                 distributed=False,
                 save_path=None):
    ...
    if(save_path!=None):
        ...
        if(distributed==True):
            #Record training
            csvlog = CSVLogger(save_path + '/training_log.csv')
            stopping = keras.callbacks.EarlyStopping(monitor='loss',
            ...
            #Fitting
            model.fit([training_images, modification_value], training_labels,
                    steps_per_epoch=steps_per_epoch_tr // hvd.size(),
                    epochs=epochs,
                    verbose=0,
                    callbacks=callbacks)
...
#Function to calculate error prediction in image
def calculate_image_metric_tfunet(test_images, modification_value, test_labels, 
                                  model, 
                                  save_path=None):

    #Using model to make prediction on test set
    p_test_labels = model.predict([test_images, modification_value], batch_size=1)
    ...
    #Error calculation
    test_error = numpy.mean( (true_matrix - predicted_matrix)**2 )/numpy.mean( true_matrix**2 )
...
#Function to calculate jacard index
def calculate_jaccard_score_tfunet(test_images, modification_value, test_labels, 
                                   model,
                                   save_path=None):
